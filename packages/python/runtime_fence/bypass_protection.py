...
def _compute_critical_hashes() -> dict:
    """Compute SHA-256 hashes of core security files at import time.
    
    In production, these should be frozen during CI/CD build.
    At runtime, self-computed hashes detect post-install tampering.
    """
    import hashlib
    import os
...
def verify_self() -> bool:
    ...
    if mismatches:
        logger.critical(
            f"INTEGRITY VIOLATION: {len(mismatches)} file(s) tampered: "
            f"{mismatches}"
        )
        return False
